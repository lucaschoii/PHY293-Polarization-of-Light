...
def cosine_squared(x, a, k, phi, b):
    return a * np.cos(k * x + phi)**2 + b

# Load data
# file_path = 'data/1/processed/cos2_2x.txt'
file_path = 'data/1/processed/cos2x.txt'
...
fitted_y = cosine_squared(x_fine, a_fit, k_fit, phi_fit, b_fit)
...
ax_main.plot(x_fine, fitted_y, '-', 
             label='Fit: $I = a \\cos^2(k  \\theta + \\phi) + b$', 
             color='red')
