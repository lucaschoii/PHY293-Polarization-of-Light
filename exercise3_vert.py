...
title = 'Vertical Polarizer: Light Intensity [V] vs Sensor Position [°]'

def arctan(x, a, k, phi, b):
    return a * np.arctan(k * np.radians(x) + phi) + b

# Load data
file_path = 'data/3/processed/vertical_polarizer.txt'
...
print(f'Max Intensity {max(y)} @ {x[np.argmax(y)]}°')
