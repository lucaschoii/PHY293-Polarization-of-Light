...
def cosine(x, I0, k, phi, b):
    return I0 * np.cos(k * x + phi) + b
...
fitted_y = cosine(x_fine, I0_fit, k_fit, phi_fit, b_fit)
...
ax_main.plot(x_fine, fitted_y, '-', 
             label=f'Fit: $I = I_0 \\cos(k  \\theta + \\phi) + b$', 
             color='red')

ax_main.set_title('Light Intensity [V] vs cos(θ) [rad]', fontsize=14)
